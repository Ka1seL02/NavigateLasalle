...
function renderCards() {
  ...
  if (activeCampus) {
    filtered = filtered.filter(
      (s) => s.campus === activeCampus || s.campus === "both",
    );
  }
  ...
  // Update toolbar text
  browseTitle.textContent = activeCategory
  ...
  function markerCount(scene) {
    return (
      (scene.navigation_markers?.length || 0) +
      (scene.info_markers?.length || 0)
    );
  }
...
function openSceneEdit(sceneId) {
  ...
  document
    .querySelectorAll("#panelScene .pfield-input")
    .forEach((el) => (el.disabled = true));

  // PSV info bar
  document.getElementById("psvInfoId").textContent = `ID: ${editingScene._id}`;
  document.getElementById("psvInfoCampus").textContent =
    `Campus: ${editingScene.campus}`;
  document.getElementById("psvInfoCategory").textContent =
...
function enterSceneEditMode() {
  ...
  if (psvMarkersPlugin) {
    ...
    allMarkers.forEach((m) => {
      try {
        psvMarkersPlugin.updateMarker({ id: m.id, draggable: true });
      } catch (e) {}
    });
  }
}
...
document
  ...
  .addEventListener("click", discardSceneChanges);

// ─── Edit Button ──────────────────────────────────────────────────────────────
document
  .getElementById("editSceneBtn")
  .addEventListener("click", enterSceneEditMode);
...
function switchPanelTab(panel) {
  ...
  document
    .getElementById("panelScene")
    .classList.toggle("hidden", panel !== "scene");
  document
    .getElementById("panelMarkers")
    .classList.toggle("hidden", panel !== "markers");
  document
    .getElementById("panelModal")
    .classList.toggle("hidden", panel !== "modal");
}
...
function prefillScenePanel() {
  ...
  document.getElementById("pSceneCategory").value =
    editingScene.category || "buildings";
...
function renderNavMarkersList() {
  ...
  container.querySelectorAll(".marker-icon-input").forEach((input) => {
    ...
    input.addEventListener("change", (e) =>
      handleMarkerIconUpload(
        e,
        input.dataset.markerId,
        input.dataset.markerType,
      ),
    );
  });
  // Wire yaw/pitch input changes
  if (isUnlocked) {
    container
      .querySelectorAll(".mrow-yaw-input, .mrow-pitch-input")
      .forEach((input) => {
        input.addEventListener("change", (e) => {
          const markerId = input.dataset.markerId;
          const navMarker = editingScene.navigation_markers?.find(
            (m) => m.id === markerId,
          );
          if (!navMarker) return;
          const yaw = parseFloat(
            container.querySelector(
              `.mrow-yaw-input[data-marker-id='${markerId}']`,
            ).value,
          );
          const pitch = parseFloat(
            container.querySelector(
              `.mrow-pitch-input[data-marker-id='${markerId}']`,
            ).value,
          );
          navMarker.yaw = yaw;
          navMarker.pitch = pitch;
          // Update marker in PSV viewer
          if (psvMarkersPlugin) {
            const marker = psvMarkersPlugin.getMarker(markerId);
            if (marker) {
              marker.setPosition({ yaw, pitch });
            }
          }
        });
      });
  ...
  container.innerHTML = markers
    .map((m, i) => {
      ...
      return `
        <div class="mrow ${m.id === selectedNavMarkerId ? "is-selected" : ""}" data-marker-id="${m.id}">
            <div class="mrow-icon mrow-icon-upload" title="Click to replace icon">
                <img src="${m.icon_url || DEFAULT_INFO_ICON}" alt="${m.id}" />
            </div>
            <div class="mrow-info">
                <div class="mrow-name">${m.tooltip || m.id}</div>
            </div>
  ...
  if (isUnlocked) {
    container
      ...
      .forEach((input) => {
        input.addEventListener("change", (e) => {
          const markerId = input.dataset.markerId;
          const infoMarker = editingScene.info_markers?.find(
            (m) => m.id === markerId,
          );
...
function updateNavMarkerSelection() {
  const containers = [
    document.getElementById("navMarkersList"),
    document.getElementById("infoMarkersList"),
  ];
  ...
  containers.forEach((container) => {
    ...
    container.querySelectorAll(".mrow").forEach((row) => {
      row.classList.toggle(
        "is-selected",
        row.dataset.markerId === selectedNavMarkerId,
      );
    });
  });
}
...
function handleMarkerIconUpload(e, markerId, markerType) {
  ...
  img.onload = () => {
    ...
    const row = document.querySelector(
      `.mrow[data-marker-id="${markerId}"] .mrow-icon`,
    );
    if (row) {
...
function initPsvViewer() {
  ...
  if (!editingScene?.panorama_url) {
    container.innerHTML =
      '<div style="color:rgba(255,255,255,0.4);font-size:0.8rem;display:flex;align-items:center;justify-content:center;height:100%;font-family:Noto Sans,sans-serif;">No panorama</div>';
    return;
  }
  ...
  psvViewer.addEventListener("ready", () => {
    ...
    addSceneMarkersToPsv(!(isUnlocked && isPositionEditMode) ? true : false);

    // Listen for marker drag
    psvMarkersPlugin.addEventListener(
      "marker-moved",
      ({ marker, position }) => {
        if (!(isUnlocked && isPositionEditMode)) return;
        // Update yaw/pitch in updatedMarkerPositions and editingScene
        const markerId = marker.id;
        let nav = editingScene.navigation_markers?.find(
          (m) => m.id === markerId,
        );
        let info = editingScene.info_markers?.find((m) => m.id === markerId);
        if (nav) {
          nav.yaw = position.yaw;
          nav.pitch = position.pitch;
          updatedMarkerPositions[markerId] = {
            yaw: position.yaw,
            pitch: position.pitch,
          };
        } else if (info) {
          info.yaw = position.yaw;
          info.pitch = position.pitch;
          updatedMarkerPositions[markerId] = {
            yaw: position.yaw,
            pitch: position.pitch,
          };
...
function addSceneMarkersToPsv(locked) {
  ...
  navMarkers.forEach((m) => {
    psvMarkersPlugin.addMarker({
      ...
      draggable: !locked && isUnlocked && isPositionEditMode,
    });
  });
  ...
  infoMarkers.forEach((m) => {
    psvMarkersPlugin.addMarker({
      id: m.id,
      position: { yaw: m.yaw, pitch: m.pitch },
      html: adminInfoMarkerHTML(m.icon_url || DEFAULT_INFO_ICON),
...
document.getElementById("saveSceneBtn").addEventListener("click", async () => {
  ...
  // If in position edit mode, turn it off before saving
  if (isPositionEditMode) {
    isPositionEditMode = false;
    if (psvViewer) addSceneMarkersToPsv(true);
    const btn = document.getElementById("toggleDragBtn");
    if (btn) {
      btn.classList.remove("active-editing");
      btn.innerHTML = `<i class='bx bx-move'></i> Edit Marker Positions`;
    }
  }
  ...
  try {
    ...
    const newSidebarLabel = document
      .getElementById("pSceneSidebarLabel")
      .value.trim();
    if (!newTitle) {
    ...
    try {
      if (newLogo) {
        updates.logo_url = await uploadSettingsFile(
          newLogo,
          "logo",
          settings.logo_url,
        );
      ...
      if (newLogoCollapsed) {
        updates.logo_collapsed_url = await uploadSettingsFile(
          newLogoCollapsed,
          "logo_collapsed",
      ...
      if (newPanoramaFile) {
        panoramaUrl = await uploadSceneFile(
          sceneId,
          newPanoramaFile,
          "panorama",
          null,
          editingScene.panorama_url,
        );
      }
      if (panoramaUrl) {
    ...
    const navMarkers = await Promise.all(
      navMarkersRaw.map(async (m) => {
        const updated = { ...m };
        const pos = updatedMarkerPositions[m.id];
        if (pos) {
          updated.yaw = pos.yaw;
          updated.pitch = pos.pitch;
        }
        if (newMarkerIcons && newMarkerIcons[m.id]) {
          try {
            // Upload new icon and set URL
            updated.icon_url = await uploadSceneFile(
              editingScene._id,
              newMarkerIcons[m.id],
              "marker_icon",
              m.id,
    ...
    const infoMarkers = await Promise.all(
      infoMarkersRaw.map(async (m) => {
        const updated = { ...m };
        const pos = updatedMarkerPositions[m.id];
        if (pos) {
          updated.yaw = pos.yaw;
...
function prefillSettings() {
  ...
  document.getElementById("welcomeBtnLabel").value =
    settings.welcome_btn_label || "";

  // Modal size fields
  document.getElementById("modalWidth").value =
    settings.welcome_modal_width || 600;
  document.getElementById("modalHeight").value =
    settings.welcome_modal_height || 400;
  ...
  toggleMediaState(
    "narrationMediaBody",
    "narrationEmptyState",
    settings.narration_url,
  );
  toggleMediaState("videoMediaBody", "videoEmptyState", settings.video_url);

  // Media sources
  if (settings.video_url)
    document.getElementById("videoPlayer").src = settings.video_url;
  ...
  if (!prefillSettings._listenersAdded) {
    document
      ...
      .addEventListener("input", updateWelcomePreview);
    document
      .getElementById("welcomeSubtitle")
      .addEventListener("input", updateWelcomePreview);
    document
      .getElementById("welcomeBtnLabel")
      .addEventListener("input", updateWelcomePreview);
    document
      .getElementById("modalWidth")
      .addEventListener("input", updateModalPreviewSize);
    document
      .getElementById("modalHeight")
      .addEventListener("input", updateModalPreviewSize);
    initResizeHandles();
...
function setImgSrc(id, url) {
  ...
  if (url) {
    el.src = url;
    el.style.display = "";
    if (fallback && fallback.classList.contains("stg-logo-fallback"))
...
function setMediaFileName(id, url, fallback) {
  ...
  if (urls.length > 1) {
    ...
    try {
      el.textContent = decodeURIComponent(url.split("/").pop());
    } catch {
...
function updateSidebarPreview() {
  ...
  if (spLogoC && settings.logo_collapsed_url)
    spLogoC.src = settings.logo_collapsed_url;
  ...
  if (spDashLabel)
    spDashLabel.textContent = settings.dashboard_label || "Dashboard";
...
function enterEditMode(cardId) {
  ...
  card
    .querySelectorAll(".stg-form-group input, .stg-size-field input")
...
document.getElementById("cancelWelcomeBtn").addEventListener("click", () => {
  ...
  if (welcomeSnapshot) {
    document.getElementById("welcomeTitle").value =
      welcomeSnapshot.welcome_title;
    document.getElementById("welcomeSubtitle").value =
      welcomeSnapshot.welcome_subtitle;
    document.getElementById("welcomeBtnLabel").value =
      welcomeSnapshot.welcome_btn_label;
    document.getElementById("modalWidth").value =
      welcomeSnapshot.welcome_modal_width;
    document.getElementById("modalHeight").value =
      welcomeSnapshot.welcome_modal_height;
    if (settingsQuill)
      settingsQuill.root.innerHTML = welcomeSnapshot.welcome_description;
  }
  updateWelcomePreview();
  updateModalPreviewSize();
  ...
  .addEventListener("click", async () => {
    ...
    try {
      ...
      const updates = {
        ...
        welcome_subtitle: document
          .getElementById("welcomeSubtitle")
          .value.trim(),
        welcome_description: description,
        welcome_btn_label: document
          .getElementById("welcomeBtnLabel")
          .value.trim(),
        welcome_modal_width:
          parseInt(document.getElementById("modalWidth").value) || 600,
        welcome_modal_height:
          parseInt(document.getElementById("modalHeight").value) || 400,
      };
      ...
      if (
        welcomeLogoInput &&
        welcomeLogoInput.files &&
        welcomeLogoInput.files[0]
      ) {
        updates.welcome_logo_url = await uploadSettingsFile(
          welcomeLogoInput.files[0],
          "welcome_logo",
          settings.welcome_logo_url,
        );
      }
...
document.querySelectorAll(".stg-sb-tab").forEach((tab) => {
  tab.addEventListener("click", () => {
    document
      .querySelectorAll(".stg-sb-tab")
      .forEach((t) => t.classList.remove("active"));
    document
      .querySelectorAll(".stg-sb-tab-content")
...
function renderBgmTracks(urlString) {
  ...
  if (urlString) {
    ...
    urls.forEach((u) => {
      let name;
      try {
        name = decodeURIComponent(u.trim().split("/").pop());
      } catch {
        name = u.trim().split("/").pop();
      }
...
function rebuildBgmList() {
  ...
  newBgmTracks.forEach((track, idx) => {
    ...
    audio.controls = true;
    audio.src =
      track.url || (track.file ? URL.createObjectURL(track.file) : "");
...
if (addBgmBtn) {
  addBgmBtn.addEventListener("click", () =>
    document.getElementById("bgmInput").click(),
  );
}
...
function renderNarrationTrack(url) {
  ...
  let name;
  try {
    name = decodeURIComponent(url.trim().split("/").pop());
  } catch {
...
if (replaceNarrationBtn) {
  replaceNarrationBtn.addEventListener("click", () =>
    document.getElementById("narrationInput").click(),
  );
}
...
function setupIconUpload(areaId, inputId, previewId, spIconId, spCIconId) {
  ...
  input.addEventListener("change", (e) => {
    ...
    reader.onload = (ev) => {
      ...
      if (sp) {
        sp.src = ev.target.result;
        sp.style.display = "";
      }
      const spC = document.getElementById(spCIconId);
      if (spC) {
        spC.src = ev.target.result;
        spC.style.display = "";
      }
    };
    reader.readAsDataURL(file);
...
setupIconUpload(
  "iconBuildingsUploadArea",
  "iconBuildingsInput",
  "iconBuildingsPreview",
  "spIconBuildings",
  "spCIconBuildings",
);
setupIconUpload(
  "iconFacilitiesUploadArea",
  "iconFacilitiesInput",
  "iconFacilitiesPreview",
  "spIconFacilities",
  "spCIconFacilities",
);
setupIconUpload(
...
setupIconUpload(
  "iconLandmarksUploadArea",
  "iconLandmarksInput",
  "iconLandmarksPreview",
  "spIconLandmarks",
  "spCIconLandmarks",
);
setupIconUpload(
  "iconParkingUploadArea",
  "iconParkingInput",
  "iconParkingPreview",
  "spIconParking",
  "spCIconParking",
...
function initResizer() {
  const resizer = document.getElementById("editResizer");
  ...
  // Markers Tab icon replacement logic
  const navMarkerIconArea = document.getElementById("navMarkerIconUploadArea");
  const navMarkerIconInput = document.getElementById("navMarkerIconInput");
  if (navMarkerIconArea && navMarkerIconInput) {
    navMarkerIconArea.addEventListener("click", () =>
      navMarkerIconInput.click(),
    );
    navMarkerIconInput.addEventListener("change", (e) => {
      if (selectedNavMarkerId)
        handleMarkerIconUpload(e, selectedNavMarkerId, "nav");
    });
  }

  // Check initial width from CSS and apply class if necessary
  const initialWidth = parseInt(window.getComputedStyle(rightPanel).width);
  if (initialWidth > 400) {
    rightPanel.classList.add("is-wide");
  }
  ...
  window.addEventListener("mousemove", (e) => {
    ...
    const containerRect = editBody.getBoundingClientRect();
    const newWidth = containerRect.right - e.clientX;

    // Apply limits (matching CSS min/max)
    if (newWidth >= 280 && newWidth <= 600) {
      rightPanel.style.width = `${newWidth}px`;

      // If panel is wide enough, add a class to scale up contents
      if (newWidth > 400) {
        rightPanel.classList.add("is-wide");
      } else {
        rightPanel.classList.remove("is-wide");
      }

      // Notify Photo Sphere Viewer to resize its canvas
      if (psvViewer) {
        psvViewer.needsUpdate();
      }
    }
  });
...
// Call this after the DOM loads or within your fetchAll()
